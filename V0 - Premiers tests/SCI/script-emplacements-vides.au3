...
Global $Nom_Table='SCI_EMPLACEMENTS_VIDES'

Local $Date = @MDAY & "-" & @MON & "-" & @YEAR

Global $Chemin_Fichier = "U:\Flux\Centrale\SCI\Exportation\"
Global $Nom_fichier_old = "FR50L.STL.007_ AIRE STOCKAGE VIDE.csv"
Global $Nom_fichier_new = "FR50L.STL.007_ AIRE STOCKAGE VIDE_MODIFIED.csv"
...
while (@error <> -1)
	...
	$EMPLACEMENT		= StringFormat("%-10.10s",StringStripWS($Zone_decoupee[1], $STR_STRIPLEADING + $STR_STRIPTRAILING))
	$NAME				= StringFormat("%-20.20s",StringStripWS($Zone_decoupee[2], $STR_STRIPLEADING + $STR_STRIPTRAILING))
	$CELLULE 			= StringFormat("%-2.2s",StringStripWS($Zone_decoupee[3], $STR_STRIPLEADING + $STR_STRIPTRAILING))
	$TYPE	 			= StringFormat("%-2.2s",StringStripWS($Zone_decoupee[4], $STR_STRIPLEADING + $STR_STRIPTRAILING))
	$INF	 			= StringFormat("%-2.2s",StringStripWS($Zone_decoupee[5], $STR_STRIPLEADING + $STR_STRIPTRAILING))
	FileWrite($hFileOpen_out,  $EMPLACEMENT &";"& $NAME &";"& $CELLULE &";"& $TYPE &";"& $INF & @CRLF)
